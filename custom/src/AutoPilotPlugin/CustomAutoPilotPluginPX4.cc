...
CustomAutoPilotPluginPX4::CustomAutoPilotPluginPX4(Vehicle* vehicle, QObject* parent)
    : PX4AutoPilotPlugin(vehicle, parent)
{
    // Whenever we go on/out of advanced mode the available list of settings pages will change
    connect(QGCCorePlugin::instance(), &QGCCorePlugin::showAdvancedUIChanged, this, &CustomAutoPilotPluginPX4::_advancedChanged);
}

// This signals that when Advanced Mode changes the list of Vehicle Settings page also changed
void CustomAutoPilotPluginPX4::_advancedChanged(bool)
{
    _components.clear();
...
const QVariantList& CustomAutoPilotPluginPX4::vehicleComponents()
{
